...
<template>
  <h1
    v-if="scheduleStore.schedulesHome.length > 0"
  ...
  <div
    v-for="(schedule, index) in scheduleStore.schedulesHome"
    class="p-5 pb-1 mb-4 border border-gray-100 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700"
    :key="index"
  >
    <div class="relative w-full border-b pb-3">
      <span
        v-if="isToday(schedule.start_date_time)"
        class="bg-primary-100 text-primary-800 text-xs font-medium mr-1 px-2.5 py-0.5 rounded dark:bg-primary-900 dark:text-primary-300"
        >Today</span
      >
    ...
    <ol class="mt-1 divide-y divider-gray-200 dark:divide-gray-700">
      <li v-for="(speaker, speakerIndex) in schedule.speakers" :key="speakerIndex">
        <div class="items-center px-3 pt-2 pb-1 sm:flex">
          <ProfileAvatar
            class="w-12 h-12 mr-1 self-start float-left rounded-full sm:mb-0 small speaker-profile"
          ...
          <div class="text-gray-600 dark:text-gray-400 w-full self-end">
            <div class="flex ml-1 max-md:text-md max-sm:text-xs font-normal mt-2 md:mt-1">
              ...
              <span class="max-md:text-md max-sm:text-xs whitespace-nowrap mr-1">
                <span class="font-medium text-gray-900 dark:text-white">Speaker:&nbsp;</span
                >{{ speaker.name }}</span
              >
            </div>
            <div class="flex ml-1 max-md:text-md max-sm:text-xs font-normal mt-2 md:mt-1">
            ...
            <div
              class="speaker-description flex self-start ml-1 max-md:text-md max-sm:text-xs font-normal mt-2 md:mt-1"
            >
              <svg
            ...
            >
              ...
              <span class="font-medium text-gray-900 dark:text-white">Audience:&nbsp;</span
              >{{ speaker.audience }}
            </div>
            <div class="w-full h-1">&nbsp;</div>
            <span
              v-if="speaker.private"
