...
<template>
  <div v-if="props.showModal" id="schedule-add-modal" tabindex="-1"
    ...
    <div class="relative w-full max-w-md max-h-full m-auto">
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        <button @click="emit('toggle', false)" type="button"
        ...
        <div class="px-6 py-6 lg:px-8">
          ...
          <h4 class="mb-4 text-md font-medium text-gray-900 dark:text-white">
            <svg xmlns="http://www.w3.org/2000/svg" height="20" viewBox="0 96 960 960" width="20" class="float-left mr-1">
              <path
  ...
  new Date(speakerStore.speakerAddSchedule.start_date_time).toLocaleTimeString()
          ...
          <div class="space-y-2">
            <div>
              <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Speaker name
              </label>
            ...
            <div>
              <label for="topic" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Topic
              </label>
              <input type="text" v-model="topic" id="topic"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                placeholder="Speaker topic" required />
            </div>
            <div>
              <div>
            ...
            <div>
              <div class="flex items-center">
                <input v-model="isPrivate" id="private" type="checkbox"
                  class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                <label for="private" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Private (Internal
                  only)</label>
              </div>
            </div>
            <div>
              <div>
                ...
                <input v-model="minutes" id="duration-range" type="range" min="5" max="60" step="5"
                  class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700" />
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ minutes }} minutes</label>
              </div>
              <button v-if="speakerStore.speakerEdit !== null" @click="updateSpeaker()" type="submit"
              ...
              <button v-else @click="addSpeaker()" type="submit"
                class="w-full text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                Save
              </button>
            </div>
