...
<template>
  <div>
    <schedule-modal
      :show-modal="showScheduleModal"
      @speaker-deleted="speakerDeleted()"
      @speaker-edit="editSpeaker()"
      @added="getSchedules(true)"
      @updated="getSchedules(true)"
      @toggle="toggleScheduleModal(false)"
    />
    <speaker-modal
      :show-modal="showSpeakerModal"
      @added="getSchedules(true)"
      @toggle="toggleSpeakerModal(false)"
    />
      ...
      <div class="px-4 mx-auto max-w-screen-xl">
        ...
        >
          ...
          <div class="flex items-center">
            ...
            />
            <label for="show-all" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
              >Show all</label
            >
          </div>
        </section>
        ...
        >
          <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                <tr
                  v-for="(schedule, index) in scheduleStore.schedulesAdmin"
                ...
                >
                  ...
                  <td class="px-3 py-2">
                    ...
                    <button
                      v-if="inFuture(schedule.start_date_time)"
                      @click="addSpeaker(schedule)"
                      :data-tooltip-target="`tooltip-speaker-{index}`"
                      type="button"
                      class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-xs px-1 py-1 mr-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                    >
                      <svg
