...
onMounted(async () => {
  const scheduleId = parseInt(String(route.params.id))
  schedule.value = await getSchedule(scheduleId)

  if (schedule.value?.speakers) {
...
<template>
  <main class="pt-8 pb-16 lg:pt-16 lg:pb-24 mb-auto bg-white dark:bg-gray-900">
    <h1
      class="text-3xl text-center font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white"
    >
      {{ schedule?.title }}
    </h1>
    <h1
      class="text-1xl text-center font-medium leading-tight tracking-tight text-gray-900 dark:text-white"
    >
    ...
    <section class="bg-white dark:bg-gray-900 antialiased mt-6">
      <div class="max-w-screen-xl px-4 pb-8 mx-auto">
        <div class="max-w-3xl mx-auto text-center">
          <h2
            class="text-2xl font-extrabold leading-tight tracking-tight text-gray-900 dark:text-white"
          >
            Agenda
          </h2>
        </div>
        <div class="flow-root max-w-3xl mx-auto mt-8">
          <div
            class="-my-4 divide-y divide-gray-200 dark:divide-gray-700 sm:text-md md:text-lg lg:text-xl"
          >
            <div
            ...
            >
              ...
              <p class="justify-self-end">
                <stop-watch
                  :max="speaker.minutes"
                  channel="speaker"
                  :socket-id="speaker.id"
                  :show-controls="authenticationStore.isLoggedIn"
                />
              </p>
            </div>
          </div>
